FamousFramework.scene('svitlana:radio:back:programs', {
	...
	events: {
		'$self': {
			'scrollback': function ($state, $dispatcher) {
				...
				if (newPos > -programWidth) {
					...
					$state.set('activeProgram', $state.get('activeProgram') - 1);
					console.log($state.get('activeProgram'));
					$dispatcher.broadcast('activeProgramChanged',$state.get('activeProgram'));
				}
			...
			'scrollforw': function ($state,$dispatcher) {
