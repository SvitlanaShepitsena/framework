var frontStates = extend(constants, {});
...
FamousFramework.scene('svitlana:radio:front', {
    ...
    behaviors: {
        '#front': {
            ...
            'origin': [0.5, 0.5],
            'mount-point': [0.5, 0.5],
            //'size': [constants.rootWidth, constants.rootWidth],
            'size': function () {
                return [constants.rootWidth, constants.rootHeight];
            },
        ...
        '#radioLogo': {
            ...
            'origin': [0.5, 0.06],
            'size': [190, 62],
            'content': '<img width="192px" src="https://s3-us-west-2.amazonaws.com/svet.com/radio/logo-50.png"/>',
            'position-z': '2'
        },
        '#radioPanel': {
            'align': [0.5, .57],
        ...
        '#programsBtn': {
            ...
            'content': `<div>PROGRAMS 2015</div>`,
            'position-z': '2',
        },
        '#sphere': {
            'size': [57, 57, 57],
            'origin': [0.5, 0.5, 0.5],
            'align': [0.226, 0.154, 0.5],
            'mount-point': [0.5, 0.5, 0.5],
            'rotation': function (rotationLive) {
                return [-Math.PI / 2, rotationLive, rotationLive];
            }
        },
    }, events: {
        '$lifecycle': {
        ...
        '#programsBtn, #liveFrontBtn, #sphere': {
            'click': function ($event, $dispatcher) {
                /*emit event to a parent*/
                $dispatcher.emit('flip');
            },
            'tap': function ($event, $dispatcher) {
                /*emit event to a parent*/
                $dispatcher.emit('flip');
            }
        },
    ...
    .config({
        includes: ['../radio.css', '../settings.js', 'famo-engine.js']
    });
